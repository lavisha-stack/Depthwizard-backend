...
# AWS Open Data Registry and exposes the conventional HGT tiles used by the
# Person 3 calibration pipeline.
...
def _tile_names(bounds: tuple[float, float, float, float]) -> list[str]:
    ...
    # Keep a small safety margin so an image exactly on a tile boundary does
    # not miss the neighbouring elevation cell.
    left -= 1e-7
    bottom -= 1e-7
    right += 1e-7
    top += 1e-7
...
def ensure_srtm_reference(
    geotiff_path: Path,
    metadata: dict,
    cache_dir: Path,
) -> Path:
    """Download/cache the SRTM tiles covering a GeoTIFF and return one raster path.

    The returned file is an EPSG:4326 GeoTIFF mosaic. Existing cached mosaics
    are reused, so repeated judge demos do not redownload the same tiles.
    """
