...
from srtm_auto import ensure_srtm_reference
...
def _run(stage: str, command: list[str]) -> None:
    ...
    try:
        completed = subprocess.run(command, check=False, capture_output=True, text=True, timeout=config.PIPELINE_TIMEOUT_SECONDS)
    except subprocess.TimeoutExpired as exc:
        raise PipelineStageError(stage, f"{label} timed out after {config.PIPELINE_TIMEOUT_SECONDS} seconds.", stdout=exc.stdout or "", stderr=exc.stderr or "") from exc
    ...
    if completed.returncode != 0:
        ...
        raise PipelineStageError(stage, message, completed.returncode, completed.stdout, completed.stderr)
...
def _failure_message(label: str, stderr: str, return_code: int) -> str:
    ...
    if missing:
        return f"{label} cannot start because Python module '{missing.group(1)}' is missing. Install requirements-pipeline.txt in the backend .venv, then restart Uvicorn."
...
def _require_outputs(stage: str, paths: tuple[Path, ...]) -> None:
    ...
    if missing:
        raise PipelineStageError(stage, f"{STAGE_LABELS.get(stage, stage)} completed without required output(s): {', '.join(missing)}.")
...
def run_person1(input_path: Path, output_dir: Path) -> None:
    _run("person1", [sys.executable, str(config.PERSON1_SCRIPT), "--input", str(input_path), "--output", str(output_dir), "--skip-original-array", "--max-model-size", str(config.MODEL_MAX_SIZE)])
    _require_outputs("person1", (output_dir / config.PERSON1_RGB_FILENAME, output_dir / "metadata.json"))
...
def run_person2(person1_dir: Path, output_dir: Path) -> None:
    ...
    args = [sys.executable, str(config.PERSON2_SCRIPT), "--input", str(rgb), "--model", config.DEPTH_MODEL]
    if mask.exists():
...
def run_person3(input_path: Path, person1_dir: Path, person2_dir: Path, output_dir: Path, srtm_path: Path | None = None, gcp_path: Path | None = None) -> None:
    ...
    args = [sys.executable, str(config.PERSON3_SCRIPT), "--geotiff", str(input_path), "--depth", str(person2_dir / config.PERSON2_DEPTH_FILENAME)]
    depth_metadata = person2_dir / "depth_metadata.json"
    if depth_metadata.exists():
        args.extend(["--depth-metadata", str(depth_metadata)])
    if source_srtm:
        args.extend(["--srtm", str(source_srtm)])
    if source_gcp:
        args.extend(["--gcps", str(source_gcp)])
    ...
    if source_metadata.get("is_georeferenced") is not True:
        ...
        raise PipelineStageError("person3", f"Absolute calibration requires trustworthy GeoTIFF georeferencing. {warning}")
    if source_metadata.get("model_width") != source_metadata.get("width") or source_metadata.get("model_height") != source_metadata.get("height"):
...
def _write_mock_heightmap(path: Path) -> None:
    ...
    for row in range(size):
        ...
        for column in range(size):
            ...
            raw_heights.append(0.9 * math.exp(-(x * x + z * z) * 0.7) + 0.32 * math.sin(x * 2.2) * math.cos(z * 1.8) + 0.45 * math.exp(-((x - 1.0) ** 2 + (z + 0.7) ** 2) * 2.0))
    low, high = min(raw_heights), max(raw_heights)
    heights = [round((value - low) / (high - low), 4) for value in raw_heights]
    path.write_text(json.dumps({"width": size, "height": size, "heights": heights, "elevation_min": 0.0, "elevation_max": 1.0, "nodata": None, "units": "relative"}, separators=(",", ":")), encoding="utf-8")
...
def _run_mock(input_path: Path, job_dir: Path) -> None:
    ...
    (p1 / "metadata.json").write_text(json.dumps({"input_type": "image", "is_georeferenced": False, "width": 2, "height": 2}, indent=2), encoding="utf-8")
    ...
    (p3 / "calibration_report.json").write_text(json.dumps({"calibration_method": "mock", "minimum_elevation": 0.0, "maximum_elevation": 1.0, "elevation_units": "relative", "is_absolute_elevation": False, "target": {"width": 33, "height": 33, "pixel_resolution": None}}, indent=2), encoding="utf-8")


def run_pipeline(input_path: Path, job_dir: Path, job_id: str, srtm_path: Path | None = None, gcp_path: Path | None = None) -> None:
    try:
        if config.MOCK_PIPELINE:
            update_status(job_dir, status="preprocessing", progress=15)
            _run_mock(input_path, job_dir)
        else:
            required_scripts = [config.PERSON1_SCRIPT, config.PERSON2_SCRIPT, config.PERSON3_SCRIPT]
        ...
        update_status(job_dir, status="failed", progress=0, stage=exc.stage, message=exc.message, return_code=exc.return_code, stdout=exc.stdout, stderr=exc.stderr)
        raise
    except Exception as exc:
