...
PERSON6_HEIGHTMAP_SCRIPT = Path(
    os.getenv(
        ...
        str(PROJECT_DIR / "depthwizard_person6" / "tools" / "prepare_heightmap.py"),
    )
).resolve()
...
# Additional origins can be supplied through the CORS_ORIGINS
# environment variable without replacing the required defaults.
...
CORS_ORIGINS = sorted(_default_cors_origins | _extra_cors_origins)
